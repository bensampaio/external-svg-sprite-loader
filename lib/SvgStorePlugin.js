...
class SvgStorePlugin {
        ...
        for (const sprite of spritesWithInterpolatedName) {
            ...
            for (const chunk of chunks) {
                for (const module of chunk.modulesIterable) {
                    this.replaceSpritePathInModule(module, sprite);
                }
            }
        }
     ...
     */
    replaceSpritePathInModule(module, sprite) {
        const source = module._source;

        switch (module.constructor.name) {
            case 'CssModule':
                module.content = sprite.replaceSpritePathInModuleSource(module.content);
                break;

            case 'NormalModule': {
                if (typeof source === 'string') {
                    module._source = sprite.replaceSpritePathInModuleSource(source);
                }
                else if (typeof source === 'object') {
                    if (typeof source._name === 'string') {
                        source._name = sprite.replaceSpritePathInModuleSource(source._name);
                    }
                    if (typeof source._value === 'string') {
                        source._value = sprite.replaceSpritePathInModuleSource(source._value);
                    }
                }
                break;
            }

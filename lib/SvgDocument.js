...
class SvgDocument {
    ...
    static create(...contents) {
        ...
        return `<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">${defs.length && `<defs>${defs.join('')}</defs>`}${$contents.html()}</svg>`;
    }
    ...
    toSymbol(id) {
        ...
        return SvgDocument.createSymbol(attrs, $svg.html().replace(/( id="|"url\(#)/g, `$1${id}`));
    }
}
